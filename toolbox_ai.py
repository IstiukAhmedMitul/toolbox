...
- Last tool: {self.context.get('last_tool', 'none')}"""
    ...
    def generate_multiple_commands(self, natural_language_request: str, num_options: int = 5) -> Tuple[bool, List[str]]:
        """
        Generate multiple command options from natural language request
        
        Returns:
            (success, list_of_commands_or_error_message)
        """
        if not self.is_groq_available():
            return (False, ["Groq API key not configured. Run 'ai-config' to set it up."])
...
Generate {num_options} different valid command options to accomplish this task.
Requirements:
- Each command should use different tools or approaches when possible
- All commands must be valid and executable
- Return ONLY the commands, one per line
- No explanations, no numbering, just the raw commands
- Commands should range from simple to advanced"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if unique_commands:
                    return (True, unique_commands[:num_options])
                else:
                    return (False, ["Could not generate valid commands"])
