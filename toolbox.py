...
"""
    ...
    def handle_ai_command(self, request: str):
        ...
        action = input("\nExecute? (y/n/e=edit/r=regenerate/f=favorites): ").strip().lower()
        
        if action == 'r':
            # Regenerate with same request
            print(f"\n[AI] 🔄 Regenerating command...")
            self.handle_ai_command(request)  # Recursive call with same request
            return
        elif action == 'e':
            # Edit mode
