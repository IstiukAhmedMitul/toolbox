...
"""
    ...
    def _offer_ai_retry(self, failed_command: str, error_output: str, exit_code: int):
        ...
        print(f"\n{Fore.YELLOW}[AI] Command failed. Would you like AI to suggest a fix?{Style.RESET_ALL}")
        choice = input(f"{Fore.CYAN}Retry with AI? (y/n): {Style.RESET_ALL}").strip().lower()
        
        if choice == 'y':
            print(f"\n{Fore.CYAN}[AI] 🤖 Analyzing error and generating fix...{Style.RESET_ALL}")
            ...
            if success:
                print(f"\n{Fore.GREEN}[AI] ✓ Suggested fix: {suggestion}{Style.RESET_ALL}")
                ...
                if retry_choice == 'y':
                    ...
                    edited = input(f"{Fore.CYAN}Edit command: {Style.RESET_ALL}").strip()
                    if edited:
                        self._execute_ai_command(edited, is_retry=True)
            else:
                print(f"\n{Fore.RED}[!] Could not generate fix: {suggestion}{Style.RESET_ALL}")
