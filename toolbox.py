...
class Toolbox:
    ...
    def _initialize_wordlists(self):
        """Initialize common wordlist locations"""
        return {
            "directory": [
    ...
    def suggest_wordlist(self, list_type):
        """Suggest wordlists based on type"""
        if list_type in self.common_wordlists:
            wordlists = self.common_wordlists[list_type]
            print(f"\n[+] Common {list_type} wordlists:")
            for i, wordlist in enumerate(wordlists, 1):
                if os.path.exists(wordlist):
                    print(f"  {i}. {wordlist} (Available)")
                else:
                    print(f"  {i}. {wordlist} (Not found)")
            
            while True:
                choice = input(f"\nSelect a wordlist (1-{len(wordlists)}) or enter custom path: ").strip()
                if choice.isdigit() and 1 <= int(choice) <= len(wordlists):
                    ...
                    if os.path.exists(choice):
                        ...
                        print(f"[!] Custom wordlist not found: {choice}")
                        continue
                else:
        ...
        try:
            while True:
                ...
                if choice == 'q' or choice == 'quit' or choice == 'exit':
                    ...
                    print("  q/quit/exit       - Exit the tool")
                elif choice == 'list':
                    self.list_all_tools()
