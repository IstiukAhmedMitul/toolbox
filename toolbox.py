...
# Import AI module
try:
    from toolbox_ai import ToolboxAI
    AI_AVAILABLE = True
except ImportError:
    AI_AVAILABLE = False
    ToolboxAI = None
...
class Toolbox:
    def __init__(self):
        ...
        self.last_output = None  # Store last command output in memory
        self.last_command = None  # Store last executed command
    ...
    def _initialize_tools_database(self):
        ...
        return {
            ...
            "netdiscover": {
                "description": "Active/passive ARP reconnaissance tool for network discovery (requires sudo)",
                "requires_target": True,
                "requires_wordlist": False,
                "commands": [
                    {
                        "command": "sudo netdiscover -i eth0 -r {target}/24",
                        "description": "Scan subnet range on eth0 interface (e.g., 192.168.1.0)"
                    },
                    {
                        "command": "sudo netdiscover -i eth0",
                        "description": "Auto scan on eth0 interface"
                    },
                    {
                        "command": "sudo netdiscover -r {target}/24",
                        "description": "Scan specific subnet range (e.g., 192.168.1.0)"
                    },
                    {
                    ...
                    {
                        "command": "sudo netdiscover -i wlan0 -r {target}/24",
                        "description": "Scan subnet on wireless interface (e.g., 192.168.1.0)"
                    },
                    {
                        "command": "sudo netdiscover -i eth0 -r {target}/24 -c 10",
                        "description": "Scan with 10 requests per discovery - faster (e.g., 192.168.1.0)"
                    },
                    {
                        "command": "sudo netdiscover -i eth0 -r {target}/24 -s 30",
                        "description": "Scan with 30 second sleep - stealthy (e.g., 192.168.1.0)"
                    },
                    {
                        "command": "sudo netdiscover -i eth0 -P",
                ...
                "commands": [
                    {
                        ...
                        "description": "Directory brute force (add http:// if needed)"
                    },
                    {
                        "command": "gobuster dir -u {target} -w {wordlist} -x php,txt,html",
                    ...
                    {
                        "command": "gobuster vhost -u {target} -w {wordlist}",
                        "description": "Virtual host enumeration"
                    },
                    {
                        "command": "gobuster dns -d {target} -w {wordlist}",
                        "description": "DNS subdomain enumeration (don't add http://)"
                    }
                ]
            },
            "dirb": {
                ...
                "commands": [
                    {
                        ...
                        "description": "Basic directory scan (add http:// if needed)"
                    },
                    {
                        "command": "dirb {target} {wordlist} -o output.txt",
                        "description": "Directory scan with output to file"
                    },
                    {
                        "command": "dirb {target} {wordlist} -X .php,.txt,.html",
                        "description": "Directory scan with specified file extensions"
                    },
                    {
                        "command": "dirb {target} {wordlist} -r",
                ...
                "commands": [
                    {
                        ...
                        "description": "Basic directory fuzzing (add http:// if needed)"
                    },
                    {
                        "command": "ffuf -w {wordlist} -u {target}/FUZZ -mc 200",
                        "description": "Directory fuzzing showing only HTTP 200 responses"
                    },
                    {
                        "command": "ffuf -w {wordlist} -u {target}/FUZZ -recursion -recursion-depth 2",
                        "description": "Recursive directory fuzzing with depth 2"
                    },
                    {
                        "command": "ffuf -w {wordlist} -u {target}/FUZZ -x http://127.0.0.1:8080",
                        "description": "Directory fuzzing through HTTP proxy"
                    },
                    {
                        "command": "ffuf -w {wordlist} -u {target}/FUZZ -fc 404",
                        "description": "Filter out 404 status codes"
                    },
                    {
                        "command": "ffuf -w {wordlist} -u {target}/FUZZ -k",
                        "description": "Ignore TLS certificate errors (use https:// in target)"
                    },
                    {
                        "command": "ffuf -w {wordlist} -u {target}/api/v1/FUZZ -mc 200,301,302",
                        "description": "API endpoint discovery with specific status codes"
                    },
                    {
                        "command": "ffuf -w {wordlist} -u {target}/FUZZ -H \"Authorization: Bearer TOKEN\"",
                        "description": "Fuzz with custom header (e.g., API key)"
                    },
                    {
                        "command": "ffuf -w {wordlist} -u {target}/FUZZ -c -v",
    ...
    def _command_needs_sudo(self, command):
        """Check if a command typically needs sudo privileges"""
        sudo_commands = ['netdiscover', 'nmap', 'masscan', 'aircrack-ng', 'tcpdump', 'wireshark']
        cmd_parts = command.strip().split()
        if cmd_parts:
            base_cmd = cmd_parts[0]
            return base_cmd in sudo_commands
        return False
    
    def _is_system_command(self, choice):
        """Check if input is a system command to pass through"""
        system_cmds = ['ifconfig', 'ip', 'whoami', 'pwd', 'ls', 'cd', 'cat', 'grep', 
                      'ps', 'top', 'df', 'du', 'free', 'uname', 'hostname', 'netstat',
                      'ss', 'route', 'arp', 'ping', 'traceroute', 'dig', 'host', 'clear', 'sudo']
        cmd_parts = choice.strip().split()
        if cmd_parts:
            base_cmd = cmd_parts[0]
            # Check if it's sudo or any system command
            return base_cmd in system_cmds or base_cmd == 'sudo'
        return False
    
    def _normalize_url(self, target):
        """Normalize URL - add http:// only if no protocol is specified"""
        target = target.strip()
        # If already has protocol, return as-is
        if target.startswith('http://') or target.startswith('https://'):
            return target
        # If it looks like a URL or IP, add http://
        return target  # Return bare target, let command template handle it
    
    def run_command(self, command, tool_name="", target=""):
        """Execute a command in the system shell with real-time output"""
        try:
            # Check if command needs sudo and we're not root
            needs_sudo = command.strip().startswith('sudo') or self._command_needs_sudo(command)
            
            # Set full PATH environment for subprocess
            import os
            env = os.environ.copy()
            env['PATH'] = '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:/usr/local/games'
            
            print(f"\033[1;36m\n[+] Executing: {command}\033[0m")
            ...
            # Store output in memory for later viewing
            self.last_output = full_output
            self.last_command = command
            
            # Save output if enabled
            self.save_output(tool_name, command, full_output, target)
            ...
            if process.returncode == 0:
                ...
                print(f"\033[1;31m[!] Command exited with code: {process.returncode}\033[0m\n")
    ...
    def interact_with_tool(self, tool_name):
        ...
        try:
            ...
            if tool_info.get("requires_target"):
                target_input = input(f"\nEnter target(s) (IP/domain/URL, comma-separated for multiple): ").strip()
                if not target_input:
                ...
                # Smart URL handling: add http:// only if no protocol specified
                # and tool is a web tool
                web_tools = ['gobuster', 'dirb', 'ffuf', 'nikto', 'wpscan', 'sqlmap', 'feroxbuster', 'wfuzz']
                if tool_name in web_tools:
                    # Check if target already has protocol
                    if not target.startswith('http://') and not target.startswith('https://'):
                        # Add http:// prefix
                        target = 'http://' + target
                        targets = ['http://' + t if not t.startswith('http://') and not t.startswith('https://') else t for t in targets]
                        print(f"[i] Added http:// prefix to target(s)")
                ...
                if choice.lower() == 'q':
                    ...
                    if len(targets) > 1:
                        ...
                        if action == 'e':
                            ...
                            while True:
                                next_action = input("\n[?] Next action? (e=edit & retry/r=run another/s=save as/q=quit): ").strip().lower()
                                if next_action == 'q':
                                    return
                                elif next_action == 'e':
                                    # Edit and retry the same command
                                    print(f"\n[+] Edit mode - Modify the command and try again:")
                                    edited_cmd = self.input_with_prefill("Command: ", formatted_cmd).strip()
                                    ...
                                    if edited_cmd:
                                        ...
                                        if confirm == 'y':
                                            self.run_command(formatted_cmd, tool_name, target)
                                            # Continue loop to ask next action again
                                        else:
                                            print("[!] Command cancelled.")
                                            break
                                    else:
                                    ...
                                    break  # Break inner loop, continue outer loop to show command list again
                                else:
                                    print("[!] Invalid option. Use e/r/s/q")
                        elif action == 'f':
                            name = input("Favorite name (optional): ").strip()
                            self.add_to_favorites(tool_name, formatted_cmd, name)
                        elif action == 't':
...
"""
        ...
        print("\033[1;35m[*] 🤖 AI-Powered Natural Language Commands (NEW!)\033[0m")
        print("\033[1;32m[*] Command History | Favorites | Workflows | Templates\033[0m")
        print("\033[1;33m[*] Custom Commands | Multi-Target | Enhanced Wordlists\033[0m")
        print("\033[1;35m[*] Hollywood Hacker Mode Included (Just For Fun!)\033[0m")
        print("\033[1;37m[*] Type 'help' for commands | 'ai-help' for AI | 'q' to quit\033[0m")
        print()
        ...
        try:
            while True:
                ...
                if choice == 'q' or choice == 'quit' or choice == 'exit':
                    ...
                    print("  view-output       - View last command output")
                    print("  save-output       - Save last command output to file")
                    print("  clear             - Clear screen and show banner")
                    print("  clear-data        - Clear history & outputs (keeps favorites/custom)")
                    
                    # Custom commands
                    print("\n[+] Custom Commands (Your Personal Commands):")
                    print("  add-custom        - Add a new custom command")
                    print("                      (Give it a name, command template, description)")
                    print("  list-custom       - List all your custom commands")
                    print("  remove-custom <name> - Remove a custom command")
                    print("  use <custom-name> - Use your custom command like any tool")
                    print("\n  Example: add-custom → my-scan → nmap -sV {target} → Quick scan")
                    print("           Then: use my-scan → Works with AI too!")
                    ...
                    self.show_config()
                elif choice == 'view-output' or choice == 'output':
                    self.view_last_output()
                elif choice == 'save-output':
                ...
                # System command passthrough (ifconfig, ls, etc.)
                elif self._is_system_command(choice):
                    print(f"\033[1;36m\n[+] Running system command: {choice}\033[0m")
                    print(f"\033[1;33m{'='*80}\033[0m")
                    ...
                    if choice in self.tools_db:
                        ...
                        print(f"\033[1;31m[!] Unknown command: {choice}\033[0m")
                        print(f"\033[1;33m[!] Type 'help' for available commands\033[0m")
                        print(f"\033[1;36m[!] Or run system commands directly (ifconfig, ls, sudo, etc.)\033[0m")
    ...
    def handle_ai_command(self, request: str):
        ...
        if not self.ai.is_groq_available():
            print("\n[!] Groq API key is not configured.")
            print("[!] Run 'ai-config' to set up your free API key.")
        ...
        success, result = self.ai.generate_multiple_commands(request, num_options=7)
        ...
        # Display options
        commands = result
        print(f"\n[AI] ✓ Generated {len(commands)} options:\n")
        ...
        while True:
            ...
            print("  1-{}: Select command by number".format(len(commands)))
            print("  r: Regenerate all options")
            print("  n: Cancel")
            ...
            if choice == 'n':
                ...
                print(f"\n[AI] 🔄 Regenerating all options...")
                self.handle_ai_command(request)
                return
            elif choice.isdigit():
                idx = int(choice) - 1
                if 0 <= idx < len(commands):
                    ...
                    print(f"[!] Invalid number. Choose 1-{len(commands)}")
            else:
                print("[!] Invalid choice. Try again.")
        
        print(f"\n[AI] ✓ Selected: {command}")
        
        # Check if requires root (basic check)
        dangerous_commands = ['rm -rf', 'dd if=', 'mkfs', '> /dev/', 'chmod 777', 'chown root']
        is_dangerous = any(danger in command for danger in dangerous_commands)
        
        if is_dangerous:
            print(f"\n\033[1;33m[!] WARNING: This command may be dangerous!\033[0m")
        ...
        if command.startswith('sudo') or any(cmd in command for cmd in ['iptables', 'systemctl', 'service ']):
    ...
    def _execute_ai_command(self, command: str, is_retry=False):
        """Execute AI-generated command"""
        # Extract tool name from command
        tool_name = command.split()[0]
        
        # Store command for output viewing
        self.last_command = command
        ...
        try:
            ...
            # Store output for view-output command
            self.last_output = full_output
            ...
            if return_code == 0:
                ...
                # Offer AI-powered retry if not already retrying
                if not is_retry and return_code != 0:
                    self._offer_ai_retry(command, full_output, return_code)
    ...
    def _offer_ai_retry(self, failed_command: str, error_output: str, exit_code: int):
        """Offer AI-powered command retry after failure"""
        ...
        if COLORAMA_AVAILABLE:
            ...
            yellow = cyan = green = red = reset = ""
        
        print(f"\n{yellow}[AI] Command failed. Would you like AI to suggest a fix?{reset}")
        choice = input(f"{cyan}Retry with AI? (y/n): {reset}").strip().lower()
        
        if choice == 'y':
            print(f"\n{cyan}[AI] 🤖 Analyzing error and generating fix...{reset}")
            
            # Build error analysis prompt
            error_prompt = f"The command '{failed_command}' failed with exit code {exit_code}.\n\nError output:\n{error_output[-500:]}\n\nSuggest a fixed command or alternative approach. Return ONLY the command, no explanation."
            
            success, suggestion = self.ai.generate_command(error_prompt)
            
            if success:
                print(f"\n{green}[AI] ✓ Suggested fix: {suggestion}{reset}")
                print(f"\nExecute? (y/n/e=edit): ", end='')
                retry_choice = input().strip().lower()
                
                if retry_choice == 'y':
                    self._execute_ai_command(suggestion, is_retry=True)
                elif retry_choice == 'e':
                    edited = input(f"{cyan}Edit command: {reset}").strip()
                    if edited:
                        self._execute_ai_command(edited, is_retry=True)
            else:
                print(f"\n{red}[!] Could not generate fix: {suggestion}{reset}")
    ...
    def show_ai_status(self):
        ...
        if status['api_configured']:
            ...
            for model in available:
                current = " (current)" if model == status['model'] else ""
                print(f"  - {model}{current}")
            
            # Show context
            ctx = status['context']
            print(f"\nCurrent AI Context:")
            print(f"- Last Target: {ctx.get('last_target', 'None')}")
            print(f"- Last Tool: {ctx.get('last_tool', 'None')}")
            print(f"- Conversation History: {len(ctx.get('conversation_history', []))} entries")
            
            print("\n[✓] AI is ready to use!")
            print("\nTry: ai scan 192.168.1.1 for vulnerabilities")
        else:
            print("\n[!] Configure your Groq API key with: ai-config")
    ...
    def configure_ai(self):
        ...
        # Configure API key (only if not already configured)
        if not status['api_configured']:
            ...
            if update_key == 'y':
                api_key = input("Enter new Groq API key: ").strip()
                if api_key:
                    self.ai.set_api_key(api_key)
        ...
        for i, model in enumerate(available, 1):
            ...
            if "8b-instant" in model:
                desc = " - Fast, recommended"
            elif "70b" in model:
                desc = " - Most capable"
            elif "mixtral" in model:
                desc = " - Large context"
            print(f"  {i}. {model}{desc}{current}")
...
""")
    ...
    def view_last_output(self):
        """View the last command's output"""
        if not self.last_output or not self.last_command:
            print("\n[!] No previous command output available.")
            print("[i] Run a command first, then use 'view-output' to see it again.")
            return
        
        print("\n" + "="*80)
        print(f"\033[1;36m[+] Last Command:\033[0m {self.last_command}")
        print("="*80)
        print(self.last_output)
        print("="*80)
        
        # Offer to save
        save = input("\n[?] Save this output to a file? (y/n): ").strip().lower()
        if save == 'y':
            self.save_last_output()
    ...
    def save_last_output(self):
        ...
        if not self.last_output or not self.last_command:
            print("\n[!] No previous command output available to save.")
            print("[i] Run a command first, then use 'save-output'.")
            return
        
        filename = input("Filename (e.g., scan_results.txt): ").strip()
        if not filename:
        ...
        try:
            ...
            with open(filepath, 'w') as f:
                f.write(f"Command: {self.last_command}\n")
                f.write("="*80 + "\n")
                f.write(self.last_output)
            print(f"\033[1;32m[+] Output saved to: {filepath}\033[0m")
        except Exception as e:
            print(f"\033[1;31m[!] Error saving file: {e}\033[0m")
    
    def clear_stored_data(self):
        """Clear history, templates, workflows, and output files (keeps favorites and custom commands)"""
        print("\n\033[1;33m[!] This will clear the following data:\033[0m")
        print("  - Command history")
        print("  - Saved outputs")
        print("  - Templates")
        print("  - Workflows")
        print("\n\033[1;32m[!] This will KEEP:\033[0m")
        print("  - Favorites (preserved)")
        print("  - Custom commands (preserved)")
        print("  - AI configuration (preserved)")
        
        confirm = input("\n[?] Are you sure you want to clear this data? (yes/no): ").strip().lower()
        if confirm != 'yes':
        ...
        try:
            ...
            # Clear in-memory output
            self.last_output = None
            self.last_command = None
            
            print(f"\n\033[1;32m[✓] Cleared: {', '.join(cleared)}\033[0m")
            print(f"\033[1;32m[✓] Favorites and custom commands preserved!\033[0m")
            
        except Exception as e:
            print(f"\033[1;31m[!] Error clearing data: {e}\033[0m")
    
    def run(self):
        """Main entry point"""
        parser = argparse.ArgumentParser(
            description="Toolbox v2.0 + AI - Professional Cybersecurity Command Assistant",
            epilog="Examples:\n"
